 ...
 * Must be used with gmdate() to be accurate.
 */
...
class OaiPmhRepository_ResponseGenerator
{
     ...
     * Default constructor
     *
     * Creates the DomDocument object, and adds XML elements common to all responses.
     */
    public function __construct()
    {
	$this->responseDoc = new DomDocument('1.0', 'UTF-8');
	$this->responseDoc->formatOutput = true;
        $root = $this->responseDoc->createElementNS(OAI_PMH_NAMESPACE_URI, 'OAI-PMH');
        $this->responseDoc->appendChild($root);
	
	$root->setAttributeNS(XML_SCHEMA_NAMESPACE_URI, 'xsi:schemaLocation', OAI_PMH_NAMESPACE_URI.' '.OAI_PMH_SCHEMA_URI);
	
	$responseDate = $this->responseDoc->createElement('responseDate', gmdate(DATE_FORMAT));
	...
	$elements = array( 'repositoryName' => get_option('oaipmh_repository_name'),
	...
	$oaiIdentifier->setAttributeNS(XML_SCHEMA_NAMESPACE_URI, 'xsi:schemaLocation', OAI_IDENTIFIER_NAMESPACE_URI.' '.OAI_IDENTIFIER_SCHEMA_URI);
